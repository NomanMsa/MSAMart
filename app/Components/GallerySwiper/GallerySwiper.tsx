...
export default class extends Component {
  ...
  updateIndex = (offset, dir, cb) => {
    ...
    //this.flatListRef.scrollToIndex({ animated: true, index: newState.index });
  }
  ...
  scrollTo = (index, animated = true, fromPaging) => {
    if (fromPaging != 'paging') {
      if (
        this.internals.isScrolling ||
        this.state.total < 2 ||
  ...
  //   let dots = []
  //   return (<>
  //     <View style={[{ width: '100%' }, this.props.paginationStyle]}>
  //       <FlatList
  //         horizontal
  ...
  renderThumbnails = ({ item, index }) => {
    const { width, height } = Dimensions.get('window');
    let border = {};
    if (index === this.state.index) { border = { borderWidth: 2, borderColor: Colors.PRIMARY, borderRadius: 5 }; }
    return <View key={index} style={[{
      ...
      <TouchableOpacity activeOpacity={ThemeConfig.activeOpacityHigh} style={{
        width: (width - 60) / 3,
        alignItems: 'center',
        height: (width - 60) / 3,
        backgroundColor: 'transparent',
        ...
        borderRadius: 4,
      }} onPress={() => { this.scrollTo(index + 1, true, 'paging') }}>
        <Image
          style={{
            width: (width - 70) / 3,
            alignItems: 'center',
