...
import styles from './VendorFilterProductListStyles';
...
class VendorFilterProductList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isOpen: false,
  ...
  async componentDidMount() {
    const vendorProductsTrace =  await perf().startTrace('custom_trace_vendor_products_screen');
    ...
    if (passData.pageName == 'filterPage') {
      ...
      await this.setState({
        ...
        // categoryId: 0,
        manufacturerId: 0,
        vendorId: this.props.Filterdata.categoryId,
        orderby: 0,
        pagesize: 15,
    ...
    vendorProductsTrace.putAttribute('occurrence', 'firstVisit');
    await vendorProductsTrace.stop();
  }
  ...
  fetchCategoriesData = async (catId) => {
    ...
    let Service = {
      apiUrl: Api.vendorFilterAPI + '?vendorId=' + catId,
  ...
  UpdateFilterData = async () => {
    let Service = {
      ...
      bodyData: JSON.stringify({
        categoryId: 0,
        manufacturerId: 0,
        vendorId: this.state.categoryId,
  ...
  onSuccessUpdateFilterDataCall = async (datas) => {
    ...
    if (this.state.PageNumber == 1) {
      ...
      let viewItemListEventParams = { 'itemList': (items).toString(), 'item_list_name': this.state.titleText, 'item_list_id': this.state.categoryId, 'item_list_type': 'vendor' };

      await analytics().logEvent('view_item_list', viewItemListEventParams);
      AppEventsLogger.logEvent(EventTags.VIEW_ITEM_LIST, viewItemListEventParams);
...
export default connect(mapStateToProps, mapDispatchToProps)(VendorFilterProductList)